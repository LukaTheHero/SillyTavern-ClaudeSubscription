...
function pickApiKeyFromAuthHeader(req) {
    ...
    // Common placeholders SillyTavern users type when "API key required" but
    // the proxy doesn't actually need one. Treat them as no-key.
    if (!key || /^(sk-no-key|none|null|placeholder|x)$/i.test(key)) return null;
    return key;
}
