...
function buildSdkOptions({ model, systemPrompt, abortController, stream, reasoningEffort, apiKey }) {
    // Use the Claude Code preset as the system-prompt base, with the user's
    // system content (SillyTavern character card etc.) appended.
    //
    // Without the preset, the SDK strips all framing and the model loses its
    // version awareness — every model answers "Sonnet" when asked
    // "are you Opus, Sonnet, or Haiku?", because it has no context on what
    // it actually is and falls back to the most-mentioned variant in its
    // training data. With the preset, identity matches the model that's
    // actually running (verified: opus->Opus, sonnet->Sonnet, haiku->Haiku).
    // This is the same mechanism the `claude` CLI uses by default, which is
    // why /model opus in the CLI gives correct self-identification.
    const presetSystemPrompt = systemPrompt
