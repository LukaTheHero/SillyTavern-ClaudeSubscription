...
function catalogEntry(baseId) {
    return CLAUDE_SUBSCRIPTION_MODELS.find((m) => m.id === baseId) || null;
}
...
export function isAdaptiveOnlyModel(id) {
    ...
    return (
        /claude-opus-4-(?:[7-9]|\d{2,})/.test(s) ||
        /claude-opus-[5-9]-/.test(s) ||
        s.includes('fable') ||
        s.includes('mythos')
    );
}
 ...
 *
 * @param {string} requested e.g. 'claude-opus-4-8', 'claude-fable-5[1m]'
...
export function parseModelRequest(requested) {
    ...
    const baseId = oneM ? raw.slice(0, -ONE_M_SUFFIX.length) : raw;
    const entry = catalogEntry(baseId);
